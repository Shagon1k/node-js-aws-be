...
describe('catalogBatchProcess function', () => {
    ...
    AWS.mock('SNS', 'publish', mockedSNSPublish);
  });
  afterAll(() => {
    AWS.restore('S3');
  });
	describe('when some records recieved', () => {
      ...
      const mockedRecords = validProducts.map(el => ({ receiptHandle: 'mockedReceipt', body: JSON.stringify(el)}));
      const mockedEvent = {
    ...
    describe('and some recieved records collect invalid product data', () => {
      const mockedRecords = validAndInvalidProducts.map(el => ({ receiptHandle: 'mockedReceipt', body: JSON.stringify(el)}));
      const mockedEvent = {
        Records: mockedRecords,
      };
      beforeAll(async (done) => {
